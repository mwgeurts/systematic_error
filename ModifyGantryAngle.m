function plan = ModifyGantryAngle(plan, degree)
% ModifyGantryAngle is a delivery plan modification plugin for the
% TomoTherapy FMEA simulation tool.  This plugin adjusts the gantry start
% angle by a given number of degrees.
%
...
%   degree: a number (or string) indicating the deviation in gantry start
%       angle, in degrees.
%
...
% Log event
Event(sprintf('Adjusting gantry start angle by %0.2f deg', degree));
...
for i = 1:size(plan.events, 1)
    ...
    if strcmp(plan.events{i,2}, 'gantryAngle') 
        ...
        plan.events{i,3} = mod(plan.events{i,3} + degree, 360);
